...
- 이 프로젝트는 **Anthropic의 공식 프로젝트가 아니며, Anthropic 또는 Claude와
  아무런 제휴·후원·인증 관계가 없습니다.** 개인이 만든 비공식(unofficial) 도구입니다.
- claude.ai 웹앱이 내부적으로만 쓰는, **문서화되지 않은(undocumented) 엔드포인트**를
  호출해서 동작합니다. Anthropic은 사전 공지 없이 언제든 이 엔드포인트의 응답 형식을
  바꾸거나, 접근 자체를 막을 수 있습니다. 그런 경우 이 앱의 일부 또는 전체 기능이
  예고 없이 동작하지 않을 수 있습니다 (자세한 내용은 아래 "비공식 API 사용에 대한
  주의사항" 섹션 참고).
